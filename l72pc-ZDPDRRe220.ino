...
int pin220 = 0; //Relay (D3 on board)
...
void loop() {
  ...
  //PWN
  int minSpeedCarb=40; //60 не не стартует
  ...
  int state220num=0;
  int count220=20;
  for (int i=0; i<count220; i++) {
    state220=digitalRead(pin220);
    if (state220==1) {
      state220num++;
    }
    delay(7); 
  }
  if (state220num==count220) {
      state220=0; //200 off
  } else {
      state220=1; //220 On
  }
  ...
  zSender.ClearItem();
   ...
   zSender.AddItem("state220", (int)state220);
   if (zSender.Send() == EXIT_SUCCESS) {      // Send zabbix items
       Serial.println("ZABBIX SEND: OK");
     } else {
      Serial.println("ZABBIX SEND: NG");
